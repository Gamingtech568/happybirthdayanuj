...
(function(){
    ...
    if(audio) {
      ...
      if(playPromise !== undefined) {
        playPromise.then(() => {
          ...
          document.addEventListener('click', function playSongOnClick() {
            audio.play();
            document.removeEventListener('click', playSongOnClick);
          }, { once: true });
        });
      }
    }
  ...
  // Play song when page loads
  if(document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', playBirthdaySong);
  } else {
    playBirthdaySong();
  }
